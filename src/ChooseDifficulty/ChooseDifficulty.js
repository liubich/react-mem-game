...
const ChooseDifficulty = (props) => {
  ...
  const difficultyOnChange = (event) => {
    props.dispatch(gameActions.onDifficultyPick(parseInt(event.target.value, 10)));
  };
  ...
  return (
    <div className="modal" style={{ visibility: props.isDifficultyVisible ? "visible" : "hidden", }}>
      <div className="modalContent">
        ...
        <input type="submit" value="Start" id="submit" disabled={!props.isButtonEnabled} onClick={onDifficultySubmit}>
        </input>
      </div>
    </div>
  )
