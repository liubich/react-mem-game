...
export function onCardClick(id) {
    return function (dispatch, getState) {
        ...
        const getOpenedCards = () => getState().cardsProperties.filter(card => card.opened);
        const getVisibleCardsNum = () => getState().cardsProperties.filter(card => !(card.hidden)).length;
        ...
        switch (getOpenedCards().length) {
            ...
            case 1:
                ...
                if (areOpenedCardsEqual()) {
                    setTimeout(() => {
                        getOpenedCards().forEach(({ id }) => dispatch({ type: "HIDE_CARD", id }));
                    ...
                    setTimeout(() => {
                        getOpenedCards().forEach(({ id }) => dispatch({ type: "CLOSE_CARD", id }));
                        dispatch({ type: "UNLOCK_FIELD" });
                    }, 600);
                }
                break;
